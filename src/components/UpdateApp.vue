...
const values = ref<IApp>({
  ...
  name: props.currentApp.name,
  description: props.currentApp.description,
  url: props.currentApp.url,
  icon: props.currentApp.icon,
})

async function handleSave() {
  try {
    await store.updateApp(values.value)
    notify.success('App data updated successfully!')
  }
  catch (error: any) {
    ...
    <QForm @submit.prevent="() => { }">
      <div flex items-center>
        <div color="#30B34A" pl-2 text-24px>
          Update App #{{ props.currentApp.id }}
        </div>
        <q-space />
      ...
      <div class="row" gap-2 p-2>
        <div class="col">
          <q-input
            v-model="values.name" outlined dense label="Name"
            :rules="[(val) => !!val || 'Name kiritilishi shart!']" hint="Name" color="positive"
          />
        </div>
        <div class="col">
          <q-input
            v-model="values.description" outlined dense label="Description"
      ...
      <div class="row" gap-2 p-2>
        <div class="col">
          <q-input
            v-model="values.url" outlined dense label="Url"
            :rules="[(val) => !!val || 'Url kiritilishi shart!']" hint="Url" color="positive"
          />
        </div>
        <div class="col">
          <q-input
            v-model="values.icon" outlined dense label="Icon"
