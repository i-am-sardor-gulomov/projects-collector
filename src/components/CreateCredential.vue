...
import type { ICredentialCreate } from '~/types'
...
const values = ref<ICredentialCreate>({
  app_id: props.appId,
  login: '',
  password: '',
})
...
async function handleSave() {
  try {
    await store.createCredential(values.value)
    notify.success('App credential created successfully!')
  }
  catch (error: any) {
    ...
    <QForm @submit.prevent="() => { }">
      <div flex items-center>
        <div color="#30B34A" pl-2 text-24px>
          Create App credential for App #{{ props.appId }}
        </div>
        <q-space />
